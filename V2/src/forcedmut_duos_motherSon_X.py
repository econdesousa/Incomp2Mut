def ForcedMut_Duos_MotherSon_X(PATH="Markers/APOAI1.txt", nMutFemale=1, nSim=100,seed=431734) :
    # ####################################################
    # 1
    ...
    import pyperclip
    pyperclip.copy(seed)
    from genFamilyAutossomes import genFamiliesXson_v2
    ...
    file_path, outFile1, outFile2, outFile3 = outFileName(save2File, "ForcedMut_Duos_MotherSon_X_nMut_"+str(nMutFemale),fp=PATH)
    ...
    for loop in range(nSim):
        ...
        father, mother, child, mutation_step, index = genFamiliesXson_v2(alleles, frequencies, nMutFemale)
